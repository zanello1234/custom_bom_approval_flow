...
class FlexibleBomWizard(models.TransientModel):
    ...
    def action_create_bom_and_return_to_sale_order(self):
        ...
        # so we'll modify the response to redirect instead
        if isinstance(result, dict) and result.get('type') == 'ir.actions.client':
            # Log the notification message for debugging
            notification_title = result.get('params', {}).get('title', 'BOM Process Completed')
            notification_message = result.get('params', {}).get('message', 'Process completed')
            _logger.info(f"BOM creation result: {notification_title} - {notification_message}")
            
            # Add a message to the sale order about the BOM creation
            try:
                self.sale_order_line_id.order_id.message_post(
                    body=f"""🔧 BOM Flexible Configurada:
...
👤 **Usuario:** {self.env.user.name}""",
                    subject="BOM Flexible Configurada - Retornando a Orden de Venta"
                )
            except Exception as e:
                _logger.warning(f"Could not post message to sale order: {e}")
        
        # Return to the sale order form view
        return {
            'type': 'ir.actions.act_window',
            'name': f'Orden de Venta {self.sale_order_line_id.order_id.name}',
            'res_model': 'sale.order',
